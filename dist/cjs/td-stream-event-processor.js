...
const ORDER_BOOK_EXCHANGE_KEYS = Object.keys(td_constants_js_1.FIELDS.ORDER_BOOK_EXCHANGE_FIELDS);
const ORDER_BOOK_EXCHANGE_VALUES = Object.values(td_constants_js_1.FIELDS.ORDER_BOOK_EXCHANGE_FIELDS);
...
const parseActivesMessage = (msg) => {
    ...
    const actives = chunk(msgData.slice(5)
        .map((i) => i.split(':').slice(3))[1], 3)
...
const parseListedBook = (data) => {
    try {
        const book = transformData(data, td_constants_js_1.FIELDS.LISTED_BOOK);
        for (let i = 0; i < book.length; i++) {
            book[i].bids = transformMessageData(book[i].bids, BID_FIELD_KEYS, BID_FIELD_VALUES);
            book[i].asks = transformMessageData(book[i].asks, ASK_FIELD_KEYS, ASK_FIELD_VALUES);
            for (let x = 0; x < book[i].bids.length; x++) {
                book[i].bids[x].bids = transformMessageData(book[i].bids[x].bids, ORDER_BOOK_EXCHANGE_KEYS, ORDER_BOOK_EXCHANGE_VALUES);
            }
            for (let x = 0; x < book[i].asks.length; x++) {
                book[i].asks[x].asks = transformMessageData(book[i].asks[x].asks, ORDER_BOOK_EXCHANGE_KEYS, ORDER_BOOK_EXCHANGE_VALUES);
            }
        }
        return book;
    }
...
class TDAmeritradeStreamEventProcessor {
     ...
     */
    constructor(emitter, handleLevelOneFeedUpdate = (data) => { }, handleLevelOneTimeSaleUpdate = (data) => { }) {
        ...
        try {
            ...
            switch (message.type) {
                case 'OrderEntryRequest':
                    parsedMessage = (0, td_notifications_js_1.parseOrderEntryMessage)(message);
                    break;
                case 'OrderFill':
                case 'OrderPartialFill':
                    parsedMessage = (0, td_notifications_js_1.parseOrderFillMessage)(message);
                    break;
                case 'UROUT':
                    parsedMessage = (0, td_notifications_js_1.parseCancelMessage)(message);
                    break;
            }
            if (parsedMessage) {
    ...
    handleQuotes(msg) {
        const data = transformData(msg, td_constants_js_1.FIELDS.LEVEL_ONE_EQUITY);
        this.emitEvent('QUOTE', data);
    }
    handleTimeSales(msg) {
        const data = transformData(msg, td_constants_js_1.FIELDS.TIMESALE);
        this.emitEvent('TIMESALE_EQUITY_UPDATE', data);
        this.handleLevelOneTimeSaleUpdate(data);
    }
    handleOptions(msg) {
        const data = transformData(msg, td_constants_js_1.FIELDS.LEVEL_ONE_OPTION);
        this.emitEvent('OPTION', data);
    }
    handleLevelOneFutures(msg, timeSales = false) {
        const data = transformData(msg, (timeSales ? td_constants_js_1.FIELDS.TIMESALE : td_constants_js_1.FIELDS.LEVEL_ONE_FUTURES));
        if (timeSales) {
    ...
    handleNews(msg) {
        const data = transformData(msg, td_constants_js_1.FIELDS.NEWS_HEADLINE);
    ...
    handleActiveOptions(msg) {
        try {
            const msgData = msg.content[0][1].split(';');
            const activeOptions = chunk(msgData.slice(5)
                .reverse()
